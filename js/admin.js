...
async function loadSecurity() {
  ...
  backupLink.href = `${API_BASE}/api/admin/security/backup`;
  backupLink.onclick = async (e) => {
    e.preventDefault();
    try {
      const token = getToken();
      const res = await fetch(`${API_BASE}/api/admin/security/backup`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      ...
      a.href = url;
      a.download = `webore-backup-${new Date().toISOString().slice(0, 10)}.db`;
      a.click();
